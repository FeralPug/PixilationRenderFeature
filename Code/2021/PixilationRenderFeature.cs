...
public class PixilationRenderFeature : ScriptableRendererFeature
{
    ...
    class DrawMeshPass : ScriptableRenderPass
    {
        ...
        public DrawMeshPass(LayerMask layerMask, PixilationSettings settings, int meshBufferID)
        {
        ...
        public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            cmd.GetTemporaryRT(meshBufferID, targetDesc);
            ...
            ConfigureTarget(meshBufferID, renderingData.cameraData.renderer.cameraDepthTarget);
        ...
        {
            //i think i will clean up the downSampleBuffer in the second pass
            //cmd.ReleaseTemporaryRT(downSampleID);
        }
    ...
    class PixilationPass : ScriptableRenderPass
    {
        ...
        public PixilationPass(LayerMask layerMask, PixilationSettings settings, int blitToBufferID, int meshBufferID, int downSampleID, string downSampleName)
        {
            this.settings = settings;
        ...
        public override void OnCameraCleanup(CommandBuffer cmd)
        {
            cmd.ReleaseTemporaryRT(meshBufferID);
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmd, _profilingSampler))
            {
                ...
                {
                    cmd.Blit(meshBufferID, downSampleIDs[0]);
                ...
                {
                    //to global set texture
                    cmd.Blit(meshBufferID, downSampleBufferID);
                }
                
                cmd.Blit(renderingData.cameraData.renderer.cameraColorTarget, blitToBufferID, settings.pixilationMaterial, PIXILATION_TO_RT_PASS);
                cmd.Blit(blitToBufferID, renderingData.cameraData.renderer.cameraColorTarget);
            }
    ...
    public override void Create()
    {
        ...
        drawMeshPass = new DrawMeshPass(settings.layerMask, settings, meshBufferID);
        //blitPass = new BlitToTargetPixilationPass(settings.layerMask, settings, downSampleBufferID, blitToBufferID, verticalBlurBufferID, horizontalBlurBufferID, meshBufferID);
        pixilationPass = new PixilationPass(settings.layerMask, settings, blitToBufferID, meshBufferID, downSampleBufferID, downSampleName);
    }
}
